    ...
    <div class="table">
        ...
        <div class="container">
            ...
            <el-table  :data="tableData" border class="table" ref="multipleTable"  @selection-change="handleSelectionChange">
                ...
                <el-table-column label="操作"  align="center"   v-if="button_role&&(button_role.delete||button_role.edit)"   width="200" >
                    <template slot-scope="scope">
                        ...
                        <el-button type="text" icon="el-icon-delete" class="red"  v-if="button_role&&button_role.delete&&scope.row.triggerStatus=='1'"   @click="handleStop(scope.$index, scope.row)" >停止</el-button>
                        <el-button type="text" icon="el-icon-edit"   v-if="button_role&&button_role.delete&&scope.row.triggerStatus=='0'"   @click="handleStart(scope.$index, scope.row)" >启动</el-button>
                        <el-button type="text" icon="el-icon-edit"   v-if="button_role&&button_role.delete"   @click="handleTrigger(scope.$index, scope.row)" >执行一次</el-button>
                        <el-button type="text" icon="el-icon-edit"   v-if="button_role&&button_role.delete"   @click="handleNextTriggerTime(scope.$index, scope.row)" >下次执行时间</el-button>
                    </template>
                </el-table-column>
            </el-table>
...
export default {
  ...
  methods: {
    ...
    async saveEdit(){
        if (this.form.id != null) {
           ...
           var res = await this.$http.post(
                baseURL_.jobUrl + "/jobinfo/update",
    ...
    async handleEdit(index, row){
         ...
         const res = await this.$http.get(baseURL_.jobUrl + "/jobinfo/getById",{
              params: { id: row.id }
         });
    ...
    async stop(id){
       const res = await this.$http.get(baseURL_.jobUrl + "/jobinfo/stop",{
              params: { id: id }
         });
    ...
    async start(id){
        const res = await this.$http.get(baseURL_.jobUrl + "/jobinfo/start",{
              params: { id: id }
         });
    ...
    async handleTrigger(index, row){
         const res = await this.$http.get(baseURL_.jobUrl + "/jobinfo/trigger",{
              params: { id: row.id }
         });
        if (res.data.code == 200) {
         this.$message("执行成功");
    ...
    async handleNextTriggerTime(index, row){
        const res = await this.$http.get(baseURL_.jobUrl + "/jobinfo/nextTriggerTime",{
    ...
    async data() {
      const res = await this.$http.get(baseURL_.jobUrl + "/jobinfo/data");
    ...
    async delete(id) {
      const del = await this.$http.get(baseURL_.jobUrl + "/jobinfo/remove", {
        params: { id: id }
      });
      this.$message("删除成功");
    ...
    // 获取 easy-mock 的模拟数据
    async getData() {
      const user = await this.$http.get(baseURL_.jobUrl + "/jobinfo/pageList", {
        params: {
          page: this.page,
