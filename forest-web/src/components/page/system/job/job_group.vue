    ...
    <div class="table">
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item><i class="el-icon-lx-cascades"></i> 执行器管理</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
  ...
  methods: {
    ...
    async handleEdit(index, row) {
      ...
      const getById = await this.$http.get(
        baseURL_.jobUrl + "/jobGroup/getById",
        {
          params: { id: row.id }
        }
      );
    ...
    async saveEdit() {
      if (this.form.id != null) {
          var res = await this.$http.post(
            baseURL_.jobUrl + "/jobGroup/update",
    ...
    async delete(id) {
      const del = await this.$http.get(baseURL_.jobUrl + "/jobGroup/delete", {
        params: { id: id }
      });
    ...
    // 获取 easy-mock 的模拟数据
    async getData() {
      const user = await this.$http.get(baseURL_.jobUrl + "/jobGroup/list", {
        params: {
          page: this.page,
